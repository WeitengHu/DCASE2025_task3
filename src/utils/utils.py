...
def extract_stereo_features(audio, sr: int = 24000, n_fft: int = 512, hop_length: int = 300, win_length: int = 512, 
                            ...
                            use_ildnorm: bool = False, use_gcc: bool = False, use_stpacc: bool = False):
    """
    ...
    if use_stpacc:
        stpacc = compute_stpacc(audio, sr=sr, n_fft=n_fft, win_length=win_length, hop_length=hop_length, nb_mels=nb_mels)
        stpacc = np.expand_dims(stpacc, axis=0)
        mel_specs = np.concatenate((mel_specs, stpacc), axis=0)
...
def compute_stpacc(audio_data, sr: int = 24000, n_fft: int = 512, win_length: int = 512,
                 hop_length: int = 300, nb_mels: int = 64):
    # short term power of autocorrelation
    mid = (audio_data[0] + audio_data[1]) / 2
    M_stft = librosa.stft(mid, n_fft=n_fft, hop_length=hop_length, win_length=win_length, window='hann', center=True)
    power = M_stft * np.conj(M_stft) # (F,T)
    acc = np.fft.irfft(power, n=n_fft, axis=0) # (n_fft, T)
    acc = acc.T  # (T, n_fft)
    if nb_mels:
        acc = np.concatenate((acc[:, -nb_mels//2:], acc[:, :nb_mels//2]), axis=-1)
    n_frame = acc.shape[0]
    ...
    for i in range(n_frame):
        acc_i = acc[i,:]
        # norm
        acc_i = acc_i / (max(abs(acc_i)) + 1e-8)
        stp = np.convolve(acc_i**2, np.hanning(8), 'same') # 8 samples at 24kHz ~= 3.3e-4sec => 10cm
        # norm
        stp = stp / (max(stp) + 1e-8)
        stpacc.append(stp)

    return np.array(stpacc)
