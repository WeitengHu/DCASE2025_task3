...
from AvgMaxPooling import AvgMaxPooling2d
...
class ResNet(nn.Module):
    def __init__(self,block,layers,num_classes=1000,zero_init_residual=False,norm_layer=nn.BatchNorm2d,include_top=True,base_channels=64):
        super(ResNet,self).__init__()
        self.in_planes = base_channels
        self.conv1 = nn.Conv2d(2,base_channels,kernel_size=7,stride=1,padding=3,bias=False)
    ...
    def forward(self,x):
        # 每个layer之后只进行frequency维度的pooling (B,C,T,F),Time-pooling在Conformer之后进行
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
